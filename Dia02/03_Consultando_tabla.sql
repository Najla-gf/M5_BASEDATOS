...
SELECT categoria,max(precio) from productos GROUP BY categoria order by 1;
...
--LINE 1: SELECT nombre_producto, max(precio) from productos GROUP BY ...

SELECT categoria, nombre_producto, max(precio) from productos GROUP BY categoria,nombre_producto;
--No se puede agrupar por nombre pq no se repiten

----TABLA VENTAS----
--1. ¿Cuál es el promedio de ventas?
SELECT * from VENTAS;
select AVG(precio_unitario) FROM ventas;
--columna TEMPORAL: para sacar el total de la venta
SELECT *,(cantidad*precio_unitario) as venta from VENTAS;
--promedio:
SELECT AVG(cantidad*precio_unitario) from VENTAS; --381,0
